...
namespace fantasysports
{
...

    {
        ...
        // DB BET 365 auslesen
        List<string> db2list5(string tabelle)
        {
            MySqlConnection connection = new MySqlConnection("server=buqhvbltjab1w9h2bks9-mysql.services.clever-cloud.com;port = 20444;database=buqhvbltjab1w9h2bks9;uid=uxayl6sbtpqdhepa;password=QZPnMNX6OIJrkYTkes3F");
            List<string> dblist5 = new List<string>();
            MySqlCommand command = connection.CreateCommand();
            MySqlDataReader Reader;
            command = connection.CreateCommand();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            List<String> Bet365 = db2list5("bet356");
            ...
            string[,] Referenz_array = list2array(Referenz, 8);
            string[,] Bet365array = list2array(Bet365, 9);
            string[,] Rotowire_array23 = list2array(Rotowire22, 33);

            //Rotowire Filtern nach Partie
            List<String> Rotowire222 = new List<String>();
            
            //Namen, Team tauschen 
            for (int i=0; i< Rotowire_array23.Length/33;i++)
            {
                for (int j=0; j<Referenz_array.Length/8;j++)
                {
                    if(Rotowire_array23[i,1]==Referenz_array[j,2])
                    {
                        Rotowire_array23[i, 1] = Referenz_array[j, 1];
                    }
                    if(Rotowire_array23[i,2]==Referenz_array[j,3])
                    {
                        Rotowire_array23[i, 2] = Referenz_array[j, 4];
                    }
                    if (Rotowire_array23[i, 3] == Referenz_array[j, 3])
                    {
                        Rotowire_array23[i, 3] = Referenz_array[j, 4];
                    }
            ...
            for (int i=0; i< Rotowire_array23.Length/33;i++)
            {
                for(int j=0; j<Fanteam_array.Length/12; j++)
                {
                    if((Rotowire_array22[i,1] == Fanteam_array[j,2] && (Rotowire_array23[i,2] == Fanteam_array[j,4] || Rotowire_array23[i,2] == Fanteam_array[j,5])) && (Rotowire_array23[i, 1] == Fanteam_array[j, 2] && (Rotowire_array23[i, 3] == Fanteam_array[j, 4] || Rotowire_array23[i, 2] == Fanteam_array[j, 5])))
                    {
                        for(int l = 0; l <33; l++)
                        {
                            Rotowire222.Add(Rotowire_array23[i, l]);
                        }
                    }
                }
            }

            string[,] Rotowire_array24 = list2array(Rotowire222, 33);
            ...
            // Multidimensional Array for Bet365
            var dfBet365 = Frame.FromArray2D(Bet365array);
            // Multidimensional Array for Referenz Array
            var dfRef = Frame.FromArray2D(Referenz_array);
            // Multidimensional Array for Fanteam
            var dfFanteam = Frame.FromArray2D(Fanteam_array);
            // Mulidimensional Array for RotowireArray
            var dfRotowire = Frame.FromArray2D(Rotowire_array22);
            ...
            {
                for (int j = 0; j < Referenz.Count() / 8; j++)
                {   //wenn NameRef==NameRot && TeamRot==TeamRef, dann hinzufügen
                    if (Rotowire_array[i, 1] == Referenz_array[j, 2] && Rotowire_array[i, 2] == Referenz_array[j, 3] && Rotowire_array[i,32] == datum)
                    {
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            string[,] Team_download = new string[temp2.Count() / 15+1, 15];
            k = 0;
            for (int i = 1; i < (temp2.Count() / 15)+1; i++)
            {
                for (int j = 0; j < 15; j++)
                {
            ...
            Team_download[0, 0] = "Spieler 1";
            Team_download[0, 1] = "Spieler 2";
            Team_download[0, 2] = "Spieler 3";
            Team_download[0, 3] = "Spieler 4";
            Team_download[0, 4] = "Spieler 5";
            Team_download[0, 5] = "Spieler 6";
            Team_download[0, 6] = "Spieler 7";
            Team_download[0, 7] = "Spieler 8";
            Team_download[0, 8] = "Spieler 9";
            Team_download[0, 9] = "Spieler 10";
            Team_download[0, 10] = "Spieler 11";
            Team_download[0, 11] = "Preis";
            Team_download[0, 12] = "Proj";
            Team_download[0, 13] = "Turnier_ID";
            Team_download[0, 14] = "Datum";



            //db auslesen
            List<string> dbListe2 = db2list("Referenz");
            string[,] temp22 = list2array(dbListe2, 7);



            for(int i=0; i<Team_download.Length/15;i++) //Teamdownload Reihen
            {
                for (int j=0;j<11;j++)                  //Spalten
                {
                    for (int l=0; l<temp22.Length/7;l++)
                    {
                        if(Team_download[i,j]==temp22[l,0])
                        {
                            Team_download[i, j] = temp22[l, 1].ToString();
                        }
                    }
                }
            }
